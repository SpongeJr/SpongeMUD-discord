...
Player.prototype.zoneChange = function(oldZone, newZone, client) {
	...
	if (affectedItems.length) {
		...
		this.sendMsg(`The following ${affectedItems.length} items ` +
...
Player.prototype.die = function(cause, battles, client) {
	...
	this.stats.deaths = this.stats.death || 0;
	this.stats.deaths[cause]++;
	
	battles[this.id].target.data.kills.push(this.id);
	
	if (battles[this.id]) {
		// was in battle when death occurred, so reset stuff
		this.inBattle = false;
		clearTimeout(battles[this.id].timer);
		battles[this.id].target.inBattle = false;
