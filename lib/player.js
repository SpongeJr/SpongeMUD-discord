...
const defaultFlee = function(attacker, exitTaken, battles) {
	...
	if (theOpponent.constructor === Player) {
		// Player
...
Player.prototype.zoneChange = function(oldZone, newZone) {
	...
	for (let itemId in this.inventory) {
		if (typeof items[itemId].zoneChange === "function") {
			...
			items[itemId].zoneChange(oldZone, newZone);
		}
	}
	...
	if (zoneList[newZone].hasPvpRooms) {
		outP += `:warning: WARNING! You have entered a zone with some rooms ` +
...
Player.prototype.whereIs = ut.defaultWhereIs;
Player.prototype.resPlant = function() {
	if (!this.allResources[this.id]) {
		//dBug(`${this.id}.resPlant(): No resources.${this.id}!`, 1);
	} else {
		// TODO: IF there are claims, AND unripes is EMPTY...
		// create unripes from claims, delete the claims
		// previously we gave gather points back here.
		// that should probably happen after gathering.
...
Player.prototype.resRipen = function() {
	if (!this.allResources[this.id]) {
		//dBug(`${this.id}.resRipen(): No resources.${this.id}!`);
	} else {
...
Player.prototype.go = function(exitTaken) {
	...
	// handle stamina
	this.stats.stamina.current -= moveCost;
...
Player.prototype.die = function(cause, battles) {
	...
	if (typeof cause === 'string') {
		...
		if (!battles[this.inBattle]) {
			dBug(`Weird! ${cause.id} killed ${this.id} outside of battle?`, 1);
		}
		//
		cause.sendMsg(outP);
	...
	dBug(`${this.id} just died from: ${causeString} in ${this.location}!`);
...
Player.prototype.getServer = function() {
	...
	if (!server) {
		dBug(`Player.getServer() just failed for ${who}, hence they shall sleep.`);
		this.sleep();
		return false;
	} else {
		return server;
...
Player.prototype.getUser = function() {
	...
	if (!user) {
		dBug(`server.members.get(${who}) is undefined!, hence they shall sleep.`);
		this.sleep();
		return false;
	}
	return user;
...
Player.prototype.registerForWorldTicks = function() {
	this.on('worldTick', () => {
		...
		if (!this.idle) {
			...
			dBug(`INFO: created .idle for players.${this.id}.`);
		}

		// stamina restoration
		let staminaMultiplier = this.posture === "sitting" ? 2.5 : 1;
		...
		// hp restoration
		let hpMultiplier = this.posture === "sitting" ? 1.5 : 1;
		...
		if (this.idle.ticks > this.idle.threshhold) {
			...
			if (this.idle.autolog) {
				...
				dBug(`[ IDLE TIMEOUT ] Just forced ${this.charName} to .sleep() (>${this.idle.threshhold} ticks)`);
			}
		}
	});
};
