...
Player.prototype.zoneChange = function(oldZone, newZone) {
	...
	if (zoneList[newZone]) {
		...
		dBug(`${this.id} just walked into unknown zone ${newZone}!`);
	}
	...
	if (this.allResources[this.id]) {
		...
		for (let roomId in claims) {
			...
			for (let rType in claims[roomId]) {
				dBug(`planting ${rType} in ${roomId} for ${this.id}`);
	...
	if (this.allResources[this.id]) {
		...
		for (let roomId in unripes) {
			...
			for (let rType in unripes[roomId]) {
				dBug(`ripening ${rType} in ${roomId} for ${this.id}`);
...
Player.prototype.getUser = function() {
	...
	if (!user) {
		dBug(`server.members.cache.get(${who}) is undefined, hence they shall sleep.`);
		this.sleep();
		return false;
	}
	return user;
