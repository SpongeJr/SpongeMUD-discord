...
const Player = function(data, allRooms, allItems, allResources) {
	...
	this.pendingDescription = data.pendingDescription || false;
	this.unsentLetter = data.unsentLetter || false;
	this.moneyPouch = data.moneyPouch;
	this.keys = data.keys;
	this.convoStates = data.convoStates || {};
	this.lastConvo = data.lastConvo || {};
	this.zoneLocker = data.zoneLocker || {};
};
Player.prototype.haskey = function(zone, keyId) {
	// returns:
	// 		false if player does not have the key, or it is out of uses
	//		true if player has the key and it's permkey or has uses left

	let haskey = false;

	if (this.keys[zone]) {
		if (this.keys[zone][keyId]) {
			if (this.keys[zone][keyId].permkey) {
				haskey = true;
			} else if (this.keys[zone][keyId].uses) {
				haskey = true;
			}
		}
	}
};
Player.prototype.usekey = function(zone, keyId) {
...
Player.prototype.zoneChange = function(oldZone, newZone) {
	...
	for (let itemId in this.inventory) {
		if (typeof items[itemId].zoneChange === "function") {
			...
			let result = items[itemId].zoneChange(oldZone, newZone, this.id);

			if (result && result.mentionIt) {
	...
	let locker = this.zoneLocker[newZone];
// {}
	...
	if (locker) {
		...
		for (let itemId in locker) {
			...
			item.zoneRetrieve(oldZone, newZone, this.id);

			let iType = item.data.type;
			returnedItems[iType] = returnedItems[iType] || 0;
			returnedItems[iType]++;
			lockerCount++;
		}

		// build list string
		for (let iType in returnedItems) {
			lockerListStr += `\n\`${iType} (x${returnedItems[iType]})\``;
		}
	}

	if (lockerCount) {
		outP += `The following ${lockerCount} zone item(s) have been returned to you:`;
		outP += lockerListStr;
	}
...
Player.prototype.sendFile = function(fname) {
	...
	let attach = new Discord.MessageAttachment(cons.DATA_DIR + fname, fname);
...
Player.prototype.sendJSON = function(obj, fname) {
	...
	let attach = new Discord.MessageAttachment(buff, fname);
	user.send('Your file, as requested:', attach);
};
...
Player.prototype.grantCurrency = function(currencyZone, amount, currencyType) {
	this.moneyPouch = this.moneyPouch || {};
	this.moneyPouch.zones = this.moneyPouch.zones || {};
	this.moneyPouch.zones[currencyZone] = this.moneyPouch.zones[currencyZone] || {};
	this.moneyPouch.zones[currencyZone][currencyType] = this.moneyPouch.zones[currencyZone][currencyType] || 0;

	this.moneyPouch.zones[currencyZone][currencyType] += amount;
};
Player.prototype.go = function(exitTaken) {
	...
	this.moveTo(newLoc, undefined, { "exitTaken": exitTaken });	// actually move us
...
Player.prototype.moveTo = function(destination, callback, data) {
	...
	eMaster('roomExit', oldLoc, who, { "newRoom": newLoc, "exitTaken": exitTaken }); // fire off roomExit, notify everyone but us
