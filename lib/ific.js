...
const handleDblUpvote = function(vote) {
	...
	if (!player) {
		console.log(`handleDblUpvote(): got a vote from ${vote.user}, but no no player record found. No reward.`);
	} else {
		...
		console.log(`handleDblUpvote(): got a vote from ${vote.user}, rewarding ${player.charName}! (${player.stats.dblVotes} total)`);
	}
};
//-----------------------------------------------------------------------------
	...
	for (let lineNum = 0; lineNum < script.length; lineNum++) {
		...
		switch (action) {
			case 'grant':
				...
				if (data.isMob) {
					dBug("parseScript(): I won't grant mobs anything.");
					break;
				}
			...
			case 'tele':
				...
				if (data.isMob) {
					dBug("parseScript(): I don't yet know how to tele mobs.");
					break;
				}
				...
				if (!rooms[newLoc]) {
					dBug(`parseScript(): tele: rooms.${newLoc} is undefined! Aborting MEHScript!`);
					return;
				}
			...
			case 'message':
				if (data.isMob) {
					dBug("parseScript(): No sense messaging a mob.");
					break;
				}
				let msg = rest[0]//rest;
				player = players[who];
			...
			case 'sendmail':
				...
				dBug('parseScript(): sendmail details: ');
				dBug(`fromString ${fromString}  toPlayerId: ${toPlayerId}  subject: ${subject}\nbody:\n${contents}`);

				if (!players[toPlayerId]) {
					dBug("parseScript(): sendmail FAILED to player ID ${toPlayerId}!", 2);
				} else {
					mail.sendMail(
						{ "from": fromString },
			...
			case 'summon':
				...
				if (whatToSummon === 'item') {
					...
					dBug(`Yo, summoning ${amount} ${mType}(s) in ${room}`);
					for (let i = 0; i < amount; i++) {
						...
						dBug(`Okay, I did it, it has the ID: ${theMob.id}`);
						//dBug(mobs[theMob.id]);
...
const defaultZoneChange = function(oldZone, newZone) {
...
const defaultZoneCrumble = function() {
	// remove from player
...
Mob.prototype.die = function(cause, battles) {
	...
	if (lootTable) {
		...
		for (let dropTableId in lootTable) {
			...
			if (!dropTable) {
				dBug(`${this.data.type} had a loot table referencing non-existent drop table ${dropTableId}!`, 2);
			} else {
				let useThisDropTableChance = lootTable[dropTableId].chance;
				dBug(`Possibly rolling on drop table ${dropTableId}... (chance ${useThisDropTableChance})`);
				if (Math.random() < useThisDropTableChance) {
					dBug("...drop table chance roll passed...");
					...
					dBug(`Rolled drop table #${tableNum} on loot table for ${this.data.type} (${pick} / ${totalFreq})`);
					...
					for (let dropNum = 0; dropNum < table.length; dropNum++) {
						...
						dBug(`Might drop ${thisDrop.amount} ${thisDrop.drop} ` +
						  `(${thisDrop.dropType}) (chance: ${thisDrop.chance})`);
						if (Math.random() < thisDrop.chance) {
							dBug("...yeah, we'll go ahead and drop that.");
							...
							switch (thisDrop.dropType) {
								...
								break;
								case "currency":
									dBug(`What currency?`, 1);
								break;
								case "script":
									dBug(`MEHScript not available yet :(`, 1);
								break;
								default:
									dBug(`What's this thing you tried to drop? A ${thisDrop.dropType}?!`, 1);
							}
						}
					}
				}
...
const buildMobs = function() {
	...
	for (let roomId in rooms) {
		for (let mobId in rooms[roomId].data.mobs) {
			...
			if (mobTemplate === "airport sewer rat") {
				dBug(`Turned a rat into undead rat in ${roomId}`);
	...
	for (let mob in mobCleanupList) {
		dBug(` Cleaned up ${mobCleanupList[mob].cleanCount} of ${mobCleanupList[mob].total} of mob ${mob}!`);
	}
};
//-----------------------------------------------------------------------------
// COMBAT
	...
	privacy: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("privacy attempted to send empty message, exception caught!");
			}
		}
	},
	autolog: {
  ...
  do: function(message, args) {
	...
	exits: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("exits attempted to send empty message, exception caught!");
			}
		}
	},
	exit: {
	...
	topxp: {
		do: function(message, args) {
			let playerArr = [];
			let outP = "";
			outP += "_Not seeing your home server here? Ask your server owner to run `m.setup`_\n";
	...
	represent: {
		do: function(message, args) {
			...
			if (!player.isRepping) {
				...
				if (!serverCfg) {
					outP += 'Sorry, the server admin for your current server needs to do the `setup` command and opt-in to the fame system.\n';
					outP += 'You can do `m.joinmud` elsewhere and `represent` a different server, or simply not generate fame for any server.';
					outP += '\nOr, check with your server owner and see if they want to enable this system with the `setup` command.';
					ut.chSend(message, outP);
					return;
				}
				if (!serverCfg.options.useFame) {
					...
					outP += 'You can do `m.joinmud` elsewhere and `represent` a different server, or simply not generate fame for any server.';
					outP += '\nOr, check with your server owner and see if they want to enable this policy with the `setup` command.'
					ut.chSend(message, outP);
					return;
				}
			}
		...
		do: function(message, args, servercfgs) {
			...
			if (who === ownerID) {
				...
				if (args[0] === "usefame") {
					...
					outP += "\nAs the server owner, you may opt-out at any time with the command `setup nofame`.";
					...
					outP += "\nAs the server owner, you may opt-in at any time with the command `setup usefame`.";
					outP += "\n **YOUR SERVER HAS NOW BEEN CONFIGURED**. Thanks so much for using SpongeMUD!";
				} else {
					...
					let fameOptInStr = "\n:information_source: **SERVER REPRESENTATION DISCLOSURE**: We have a 'server representation' system that you, the server owner, may opt into.";
				...
				outP += "Only the server owner may run the setup command.";
			}
			ut.chSend(message, outP);
		}
	},
	joinmud: {
		do: function(message, args, servercfgs) {
			...
			if (typeof player === 'undefined') {
				...
				if (!server) {
					...
					if (!servercfgs.servers.hasOwnProperty(server.id)) {
						outP += "\nThis server Admin has not set up SpongeMUD. You can't make a character from " +
						  "this server until they have done so. Please tell your server owner to do the command `" + cons.PREFIX +
						  "setup` and follow the instructions so that everyone on this server can join the fun!";
						failed = true;
					}
				}
			...
			if (server && !servercfgs.servers.hasOwnProperty(server.id)) {
				outP += "\nThis server Admin has not set up SpongeMUD. You can't join the game from " +
				  "this server until they have done so. Please tell your server owner to do the command `" + cons.PREFIX +
				  "setup` and follow the instructions so that everyone on this server can join the fun!";
				ut.chSend(message, outP);
				return;
	...
	wield: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("wield attempted to send empty message, exception caught!");
			}
		}
	},
	unwield: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("unwield attempted to send empty message, exception caught!");
			}
		}
	},
	attack: {
		...
		do: function(message, args) {
			...
			*/
			...
			if (outP) {
				...
				console.log("exam attempted to send empty message, exception caught!");
			}
		}
	},
	nukemyzone: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("nukemyzone attempted to send empty message, exception caught!");
			}
		}
	},
	get: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("get attempted to send empty message, exception caught!");
			}
		}
	},
	wizget: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("wizget attempted to send empty message, exception caught!");
			}
		}
	},
	drop: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("drop attempted to send empty message, exception caught!");
			}
		}
	},
	crush: {
	...
	use: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("use attempted to send empty message, exception caught!");
			}
		}
	},
	inv: {
	...
	makeprop: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("makeprop attempted to send empty message, exception caught!");
			}
		}
	},
	edtemp: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("edtemp attempted to send empty message, exception caught!");
			}
		}
	},
	wiztemp: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("wiztemp attempted to send empty message, exception caught!");
			}
		}
	},
	publish: {
	...
	wizitem: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("wizitem attempted to send empty message, exception caught!");
			}
			ut.saveObj(rooms, cons.MUD.roomFile);
			ut.saveObj(players, cons.MUD.playerFile, {getData: true});
		}
	...
	killitem: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("killItem attempted to send empty message, exception caught!");
			}
		}
	},
//-----------------------------------------------------------------------------
	profile: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("profile attempted to send empty message, exception caught!");
			}
		}
	},
	setprofile: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("setprofile attempted to send empty message, exception caught!");
			}
		}
	},
	title: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("title attempted to send empty message, exception caught!");
			}
		}
	},
	approve: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("approve attempted to send empty message, exception caught!");
			}
		}
	},
	deny: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("deny attempted to send empty message, exception caught!");
			}
		}
	},
	build: {
	...
	icanhaz: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("icanhaz attempted to send empty message, exception caught!");
			}
		}
	},
	mute: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("mute attempted to send empty message, exception caught!");
			}
		}
	},
	unmute: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("unmute attempted to send empty message, exception caught!");
			}
		}
	},
	setaccess: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("setaccess attempted to send empty message, exception caught!");
			}
		}
	},
	recall: {
	...
	pcalc: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("pcalc attempted to send empty message, exception caught!");
			}
		}
	},
	survey: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("survey attempted to send empty message, exception caught!");
			}
		}
	},
	gather: {
	...
	claim: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("claim attempted to send empty message, exception caught!");
			}
		}
	},
	recipe: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("recipe attempted to send empty message, exception caught!");
			}
		}
	},
	craft: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("craft attempted to send empty message, exception caught!");
			}
		}
	},
	setmacro: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("setmacro attempted to send empty message, exception caught!");
			}
		}
	},
	menu: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("menu attempted to send empty message, exception caught!");
			}
		}
	},
	macro: {
	...
	game: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("game attempted to send empty message, exception caught!");
			}
		}
	},
	mail: {
		do: function(message, args) {
			...
			if (subCmd === "read") {
				...
				outP += "Once there, type `write <message>` to to compose your letter. ";
			...
			if (outP) {
				...
				console.log("mail attempted to send empty message, exception caught!");
			}
		}
	},
	write: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("write attempted to send empty message, exception caught!");
			}
		}
	},
	post: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("post attempted to send empty message, exception caught!");
			}
		}
	},
	friend: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("friend attempted to send empty message, exception caught!");
			}
		}
	},
	unfriend: {
		do: function(message, args) {
			...
			if (outP) {
				...
				console.log("unfriend attempted to send empty message, exception caught!");
			}
		}
	}
};
