...
const cantDo = function(who, action, data, client) {
	...
	switch (action) {
		...
		case 'wizmob':
			...
			if (!player.isZoneAuthor(rooms) && !player.isAtLeast('sponge')) {
...
const battleSegment = function(battle, client) {
	...
	if (player.weapon) {
		...
		let delay = cons.COMBAT.defaultUnarmed.delay;
		let dmg = cons.COMBAT.defaultUnarmed.dmg;
		let hitChance = cons.COMBAT.defaultUnarmed.hitChance;
		let type = cons.COMBAT.defaultUnarmed.dmgType;
		...
		if (battle.segment % delay === 0) {
			...
			if (nextAction[0] === 'flee') {
				...
				battleStr += `**${player.charName}** attacks **${target.data.type}** with its ${weapon}`;
				if (Math.random() < hitChance) {
					...
					if (Math.random() < evadeChance) {
						...
						if (damageDealt) {
							battleStr += ` and hits for ${damageDealt} damage (${type})!`;
						}
						
						target.takeDamage(player, damageDealt, type);
	...
	if (battle.segment % target.data.attack.delay === 0) {
		...
		let type = target.data.attack.dmgType;
		if (Math.random() < hitChance) {
		...
		do: function(message, args) {
			...
			if (idata.family === "prop") {
				...
				outP += ` Gave you a ${theItem.data.type}(${theItem.data.shortName}) prop with id ${theItem.id} .\n :warning:` +
				  " When you drop it, you won't be able to pick it up with `get`. Use `wizget` to pick it up if you need!";
			} else {
				theItem = new Item(iType, {
					"hidden": idata.hidden,
					"shortName": idata.shortName,
