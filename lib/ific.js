...
const Battle = function(data) {
	...
	this.nextAction = data.nextAction || (() => {
		let nextAction = {};
		nextAction[this.attacker] = 'attack';
		nextAction[this.target.id] = 'attack';
		return nextAction;
	})();
...
const battleSegment = function(battle, client) {
	...
	if (weapon) {
		// TODO: like weapon stuff here
		delay = weapon.data.delay;
		dmg = weapon.data.dmg;
		hitChance = weapon.data.hitChance;
		...
		hitChance = cons.COMBAT.defaultUnarmed.hitChance;
	...
	if (battle.segment % delay === 0) {
		...
		if (nextAction[0] === 'flee') {
			...
			if (Math.random() < hitChance) {
				let evadeChance = target.data.evadeChance;
				if (Math.random() < evadeChance) {
					battleStr += ` but **${target.data.type}** dodges!`;
				} else {
	...
	if (battle.segment % target.data.attack.delay === 0) {
		...
		let dmg = target.data.attack.dmg;
		let hitChance = target.data.attack.hitChance;
		let dmgType = target.data.attack.dmgType;
		if (Math.random() < hitChance) {
			let evadeChance = player.stats.evadeChance || cons.COMBAT.defaultEvade;
			if (Math.random() < evadeChance) {
				battleStr += ` but **${player.charName}** dodges!`;
			} else {
				let damageDealt = ut.rollDice(dmg);
				battleStr += ` and hits for ${damageDealt} damage (${dmgType})!`;
...
module.exports = {
	...
	flee: {
		do: function(message, args, client) {
			...
			if (args[0] === '') {
				if (theMob.data.allowsFleeWithoutDirection) {
			...
			battles[who].nextAction[who] = `flee ${exitTaken}`;
		}
	},
	exits: {
		do: function(message, args, client) {
			...
			if (!numFound || firstPart > numFound || firstPart < 0) {
				...
				if (choiceList[choiceNum].where === 'char') {
					...
					if (theMob.data.isEssential || !theMob.data.attack) {
						...
						if (battles[who]) {
							outP += `You're already engaged in combat with ${battles[who].target.data.type}!`;
						} else if (theMob.inBattle) {
							outP += `**${theMob.data.type}** is already in battle against **${players[theMob.inBattle].charName}**!`;
						} else {
							pl.inBattle = true;
							theMob.inBattle = who;
