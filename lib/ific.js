...
const cantDo = function(who, action, data) {
	...
	switch (action) {
		...
		case 'go':
			...
			if (player.stats.stamina < player.stats.moveCost) {
...
var Player = function(data) {
	...
	this.stats.stamina = this.stats.stamina || 200;
	this.stats.maxStamina = this.stats.maxStamina || 200;
	this.stats.staminaPerTick = this.stats.staminaPerTick || 40;
	this.stats.moveCost = this.stats.moveCost || 10;
		...
		do: function(message, args, client) {
			...
			if (typeof rooms[pLoc].data.exits[where] !== 'undefined') {
				if (!rooms[pLoc].data.exits[where].goesto) {
					...
					player.stats.stamina -= player.stats.moveCost;
