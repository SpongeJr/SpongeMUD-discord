...
Player.prototype.registerForWorldTicks = function(client) {
	...
	this.on('worldTick', function() {
		...
		//dBug(`${player.id} got a worldTick!`);
		...
		player.idle.ticks++;
		//dBug(`${player.charName}.idle.ticks = ${player.idle.ticks} / ${player.idle.threshhold}`);
			...
			*/
			...
			if (!numFound || firstPart > numFound || firstPart < 0) {
				...
				if (choiceList[choiceNum].where === 'char') {
					outP = players[choiceList[choiceNum].ids[0]].describeAs(pl);
