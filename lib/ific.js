...
cons.HOURS_IN_DAY = 24; // default 24
...
let titleList = ["", "the Noob", "the Explorer", "the Adventurer", "the Experienced", "the Creative", "the Wizardly", "the Brave", "the Cowardly", "the Quester", "the Immortal", "the Janitor", "the Exterminator"];
let dreamStrings = {
...
//-----------------------------------------------------------------------------
let eMaster = function(eventName, where, sender, data, client) {
...
Player.prototype.registerForWorldTicks = function(client) {
	...
	this.on('worldTick', function() {
		...
		player.stats.stamina = Math.min(player.stats.stamina + staminaRestored, player.stats.maxStamina);
		
		// hp restoration
		let hpMultiplier = player.posture === "sitting" ? 1.5 : 1;
		let hpRestored = player.stats.hpPerTick * hpMultiplier;
		player.stats.hp = Math.min(player.stats.hp + hpRestored, player.stats.maxHp);
...
let buildPlayers = function(client) {
	...
	for (let player in players) {
		...
		let thePlayer = new Player(players[player]);
