...
BattleParticipant.prototype.checkDeath = function(battle) {
	let death = false;
	if (this.participant.constructor === Mob) {
	...
	if (death) {
		...
		// TODO: handle mob cause of death lacking client variable
		this.participant.die(battle.participants[this.lastAggressor].getName(), battles, battle.participants[this.lastAggressor].participant.client);
...
const newBattleSegment = function(battle, client) {
	...
	battle.battleOrder.forEach(participant => {
		if (battle.participants[participant].checkDeath(battle)) {
