		...
		do: function(message, args) {
			...
			if (!player.isRepping) {
				if (CONFIG.noRepServers.hasOwnProperty(lastServerId)) {
					...
					outP += '\nOr, check with your server admin and see if they want to enable this policy with the `serversetup` command.'
					ut.chSend(message, outP);
					return;
				}
			}
	...
	joinmud: {
		do: function(message, args) {
			...
			if (typeof player === 'undefined') {
				...
				let newCharChanText = `\`${new Date()}\`: A new character has entered the SpongeMUD World! Welcome, \`${charName}\`!`;
		...
		do: function(message, args) {
			...
			if (!args) {
				...
				if (!match) {
					...
					target.mute();
					let muteModeratorText = `\`${new Date()}\`: ${message.author.username} has muted ${target.charName} in room ${target.location}`;
					...
					message.client.channels.cache.get(cons.NEWCHARCHAN_ID).send(muteModeratorText);
		...
		do: function(message, args) {
			...
			if (!args) {
				...
				if (!match) {
					...
					target = players[match];
					target.unmute();
					let unmuteModeratorText = `\`${new Date()}\`: ${message.author.username} has unmuted ${target.charName} in room ${target.location}`;
					outP += `${args} is now able to use \`say\` \`yell\` and \`me\` commands again.`;
					outP += "\nIncident will be written to #moderation-log of SpongeMUD-dev.";
					outP += "\nPlease followup with your moderation teammates if necessary!";
					message.client.channels.cache.get(cons.NEWCHARCHAN_ID).send(unmuteModeratorText);
				}
			}
			if (outP) {
				ut.chSend(message, outP);
