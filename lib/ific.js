		...
		do: function(message, args, client) {
			...
			if (typeof rooms[pLoc].data.exits[where] !== 'undefined') {
				if (!rooms[pLoc].data.exits[where].goesto) {
					...
					let newLoc = rooms[pLoc].data.exits[where].goesto; // find our target room
					chanStr += player.go(newLoc, client); // actually move the character
				}
			} else {
	...
	exit: {
		do: function(message, args, client) {
			...
			if (isNaN(choice) || choice < 1 || !exits[choice - 1]) {
				...
				let newLoc = room.data.exits[exits[choice - 1].exName].goesto;
			
				outP += player.go(newLoc, client);
			}
			ut.chSend(message, outP);
		}
	},
