...
const parseScript = function(script, data) {
	...
	for (let lineNum = 0; lineNum < script.length; lineNum++) {
		...
		switch (action) {
			...
			break;
			case 'summon':
				let whatToSummon = rest.split(' ')[0];
				if (whatToSummon === 'item') {
					rest = rest.replace('item ', '');
					let iType = rest.split('"')[1];
					let amount = rest.split('"')[2].trim();
					if (amount === "") {
						amount = 1;  // default
					} else {
				...
				if (Math.random() < move.chance) {
					...
					if (choices.length > 0) {
						...
						*/
						...
						eMaster('roomExit', this.data.location, {"sayFrom": sayFrom}, {"newRoom": choice.goesto, "exitTaken": choice});
						eMaster('roomEnter', choice.goesto, {"sayFrom": sayFrom}, { "lastRoom": this.data.location, "exitTaken": choice });
					...
					for (let dropNum = 0; dropNum < table.length; dropNum++) {
						...
						if (Math.random() < thisDrop.chance) {
							...
							switch (thisDrop.dropType) {
								case "item":
									parseScript([
											`summon item ${thisDrop.drop} ${amountRolled}`,
											`message You found ${amountRolled} ${thisDrop.drop}!`
								...
								case "material":
									...
									parseScript([
										`grant material ${materialType} ${materialRarity} ${amountRolled}`,
										`message You found ${amountRolled} of ${materialStr} ${materialType}!`
									], { who: cause.id, room: this.data.location, isMob: mobKill });
								break;
								case "currency":
...
BattleParticipant.prototype.takeDamage = function(damageDealt, dmgType) {
	if (this.participant.entityType === cons.ENTITIES.mob) {
		...
		parseScript([`grant stat hp -${damageDealt}`], {"who": this.participant.id});
	}
};
...
//-----------------------------------------------------------------------------
	...
	wizmob: {
		do: function(message, args) {
			...
			parseScript(`summon mob "${mType}"`, { room: player.location });
...
//-----------------------------------------------------------------------------
	...
	mail: {
		do: function(message, args) {
			...
			if (subCmd === 'read') {
				...
				outP += "\nThen, use the command post <character> <subject> to send your letter off!";
	...
	post: {
		do: function(message, args) {
			...
			// TEST -- SO FAR SO GOOD!)
			/* 
			outP += "SENDING A TEST LETTER TO MEDDLON!";
			mail.sendMail(
				{ "from": player.charName },
				{ "to": players["167711491078750208"] },
				{
					"subject": "Helping you test!",
					"contents": `Hi Meddlon, just thought I'd help you test!\n\n   -${player.charName}`
				},
				world.time.tickCount
			);
			*/
			...
			if (!recipient) {
				...
				if (!subject) {
					...
					if (!letter) {
						...
						// TODO: More proper checks here, like privacy checks
						// for now, we're just saying, if there was a match, it's a valid recipient
						validRecipient = match;
						...
						if (!validRecipient) {
							...
							outP += "You can try again with `post <recipient> <subject>`."
						} else {
							mail.sendMail(
								{ "from": player.charName },
								{ "to": players[match] },
