...
var titleList = ["", "the Noob", "the Explorer", "the Adventurer", "the Experienced", "the Creative", "the Wizardly", "the Brave", "the Cowardly", "the Quester", "the Immortal", "the Janitor"];
var dreamStrings = {
...
const cantDo = function(who, action, data, client) {
	...
	switch (action) {
		...
		case 'edroom': 
			if (!player.isZoneAuthor()) {	
				if (!player.isWearing('The Omniring')) {
					let outStr = "Your Wizardship, you need to `edroom` while standing in a zone you author.\n";
					...
					let outStr = "Apologies, Your Wizardship, but I must inform you: \n";
...
var SceneryItem = function(itemType, data) {
	...
	dBug(`${itemType}: ${this.data.shortNames}`);
...
Mob.prototype.die = function(cause, client) {
	...
	if (typeof cause === 'string') {
		...
		player.sendMsg(`** RECEIVED ${xpAmt} XP FOR DEFEATING ${this.data.type}!**`, client);
	}
		...
		for (let phrase in triggers) {
			...
			if (phrase !== 'ELSE') {
				this.on('roomSay', function(whoSaid, whatSaid, client) {
					dBug(`whatSaid: ${whatSaid}    trigger phrase: ${phrase}`);
					if (whatSaid === phrase) {
						dBug(phrase);
						...
						switch (action) {
							case 'grant': 
								...
								if (whatToGrant === 'title') {
									titleNum = parseInt(rest.replace('title ', ''), 10);
									players[whoSaid].unlockTitle(titleNum);
									// to do: figure out player congrats stuff
								}
							break;
							case 'tele': {
								let target = rest;
								...
								ut.saveObj(players, cons.MUD.playerFile);
								// ut.chSend(message, ' You teleport!'); // no message here
...
Player.prototype.weighMe = function() {
	...
	dBug(`${this.charName} weighs ${totalWeight}`);
	return totalWeight;
};
...
Player.prototype.registerForWorldTicks = function(client) {
	...
	this.on('worldTick', function() {
		...
		if (!player.stats.dayTicks) {
			...
			dBug(`Setting ${player.charName}.stats.dayTicks for the first time!`);
		}		
	...
	for (let roomId in rooms) {
		for (let mobId in rooms[roomId].data.mobs) {
			...
			if (!mobTypes[mobTemplate]) {
				dBug(`Room ${room} contained a mob of non-existent type ${mobId} - ignoring!`, 1);
			} else {
	...
	if (now.remain === 0) {
		...
		if (now.hour === 6) {
			...
			for (let player in players) {
				...
				if (typeof players[player].stats.dayTicks === 'undefined') {
					players[player].stats.dayTicks = 0;
				}
				let dayTicks = players[player].stats.dayTicks;
				let maxXpTicks =  cons.DEFAULTS.maxXpTicks;
				let xp = Math.min(dayTicks, maxXpTicks) * cons.DEFAULTS.xpPerTick;
				if (!players[player].stats.xp) { players[player].stats.xp = 0; }
				players[player].stats.xp += xp;
				players[player].stats.dayTicks = 0;
			}
		}
	}
		...
		do: function(message, parms, client) {
			...
			if (typeof player === 'undefined') {
				...
				if (charName.length < 3 || charName.length > 15) {
					outP += message.author.username + ', use `joinmud <character name>`.' +
				...
				if (!server) {
					outP += '\nSince this is your first time playing, you will ' +
					  'have to do `m.joinmud` in a channel rather than DMing. After that, ' +
					  'you can use DM from then on. We just need to set your "home server".';
					failed = true;
				}
				...
				outP += ' Generally, you\'ll be using DM with me to experience the MUD world. This usually means a lot of DMs!';
				outP += '\n When you are through playing for now, do `exitmud` (in a DM with me) if you want to stop receiving game DMs.';
				outP += '\n You will then see no more DMs unless you `joinmud` again. If you do not log out with exitmud, you will be logged'
				outP += '\n out after a period of no activity from you. You can toggle or change this behavior with `autolog` (in a DM with me)';
			...
			if (typeof player.server === 'undefined') {
				...
				outP = `You are now logged in via **${server.name}**   Last: **${lastServer.name}**\n`;
				outP += MUDnews.welcome;
				outP += '\n**LATEST NEWS:**\n'
				outP += MUDnews.new;
				outP += '\n';
				ut.chSend(message, outP);
		...
		do: function(message, args, client) {
			...
			if (!numFound || firstPart > numFound || firstPart < 0) {
				...
				if (choiceList[choiceNum].where === 'char') {
					outP += `${choiceList[choiceNum].ids[0]} is a character. `;
