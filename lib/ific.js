...
var Player = function(data) {
	...
	/*
	let backpack = new Item("unique", {
			description: "You can keep some stuff in here safely.",
			shortName: "backpack",
			location: this.location
	});
	items[backpack.id] = backpack;
	*/
...
var buildItems = function() {
	...
	for (let player in players) {
		...
		for (itemId in players[player].inventory) {
			...
			if (!itemTypes[iType]) {
				...
				let theItem = new Item(iType, {
					"shortName": itemTypes[iType].shortName,
					"description": itemTypes[iType].description,
					"location": player
				});
				items[theItem.id] = theItem;
			}
			
			//items[itemId] = new Item(players[player].inventory[itemId], 
		}
	}
	...
	drop: {
		do: function(message, parms, client) {
			...
			if (choices.length === 1) {
				...
				targetId = choices[choiceNum];
				ut.chSend(message, `Found ${choices.length} matches, using [${choiceNum}] \`(${targetId})\``);
			}
