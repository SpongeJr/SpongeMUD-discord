...
const battleSegment = function(battle, client) {
	...
	if (battleStr !== '') {
		if (playerDeath) {
			battleStr += `\n _${player.charName} defies death and fights on!_\n`;
		}
		player.sendMsg(battleStr, client);
	}
	
	if (targetDeath) {
		target.die(player, client);
...
module.exports = {
	...
	flee: {
		do: function(message, args, client) {
			...
			let fleeSuccessChance = 0.5;
			
			if (Math.random() < fleeSuccessChance) {
				player.inBattle = false;
				theMob = battles[who].target;
				theMob.inBattle = false;
				clearTimeout(battles[who].timer);
				delete battles[who];
				ut.chSend(message, "You have disengaged from combat!");
			} else {
				ut.chSend(message, "You were unable to flee!");
				return;
			}
		}
	},
