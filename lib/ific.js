...
const createScript = function(command, args) {
    return command + " " + args.map(x => '"'+x.replace(/"/g, '""')+'"').join(" ");
};
...
MobGenerator.prototype.generate = function() {
	...
	if (mobCount >= max) {
		...
		let theMob = parseScript(createScript("summon mob", [mType]), { room: this.data.generator.pops });//parseScript(`summon mob "${mType}"`, { room: this.data.generator.pops });
		// .source ??? appears to be handled after the fact as well as in (old) constructor, hmm
		//


		// hmm, ok
		// you have some code for that already, right?

		// sent via discord, but that means we need to refactor parseScript to accept tokens in quotes...
		// I believe it does... look a couple lines up?

		// I think we should do a wrapper for parseScript
		// basically I think we should standardise parseScript to use "".
		// and then the wrapper helps pass everything in safely quoting if needed
		// not yet but quick to code.
		/*
		let mdata = mobTypes[mType].data;
