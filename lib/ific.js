...
for (let moduleName in moduleList) {
	...
	console.log(`>> LOADED MODULE: "${moduleName}" from: ${modulePath + moduleList[moduleName]}`);
}
...
const getMail = modules.mail.getMail;
const craft = modules.craft;

// TODO: Refactor these into constants.js
...
//-----------------------------------------------------------------------------
const defaultDecay = function(client) {
...
const defaultFoodUse = function(who, loc, client) {
	let player = players[who];
...
const defaultEquip = function(player, client) {
	//let player = players[who];
	let weapon = player.weapon;
	player.weapon = this.id;
	let sayFrom = player.charName;
	let phrase = `equips ${this.data.type}`;
	eMaster('roomGeneric', player.location, {"sayFrom": sayFrom}, phrase, client);
};
const defaultGet = function(who, client) {
	this.unregisterForWorldTicks();
...
const defaultUnwield = function(actor, client) {
	...
	eMaster('roomGeneric', actor.location, {"sayFrom": sayFrom}, phrase, client);
}
const defaultDrop = function(who, where, client) {
...
const defaultCrush = function(who, where, client) {
			...
			case 'grant':
				...
				if (whatToGrant === 'title') {
					...
					if (!player.stats.hasOwnProperty(statName)) {
						...
						dBug(`parseScript(): Changed ${player.charName}'s ${statName} by ${amt} to ${player.stats[statName].current}`);

						player.sendMsg(`**${statName}**: ${player.stats[statName].current}`, client);
					...
					if (legit) {
						...
						dBug(`granting ${amount} ${rarity} ${material}...`);
					...
					dBug(`MEHScript error on line ${lineNum}): Can't grant ${whatToGrant}!`);
				}
			break;
			case 'tele':
				let target = rest;
				...
				eMaster('roomExit', pLoc, who, {"newRoom": newLoc}, client); // fire off roomExit, notify everyone but us
				...
				eMaster('roomEnter', newLoc, who, { "lastRoom": oldLoc }, client); // fire off roomEnter, notify everyone + us
...
const defaultZoneChange = function(oldZone, newZone, client) {
	//dBug(`I am a ${this.data.type} and I just changed from ${oldZone} to ${newZone}.`);

	if (oldZone === this.data.zone) {
		if (!this.data.global) {
			this.zoneCrumble(client);
		} else {
			dBug(`A ${this.data.type} refused to .zoneCrumble() because it's a global item.`);
		}
	}
};
const defaultZoneCrumble = function(client) {
	// remove from player
	...
	let player = players[this.data.location];
...
// MOBS
//-----------------------------------------------------------------------------
const Mob = function(mobTemplate, data, client) {
	this.data = Object.assign({}, data); // break first level of references
...
Mob.prototype.timedActions = function(tickCount, client) {
	...
	// .move
	if (this.inBattle) {
		dBug(`${this.data.type} wanted to .move() but is in battle with ${this.inBattle} and can't!`);
	} else {
			...
			if (move.behavior === "random") {
				if (Math.random() < move.chance) {
					...
					if (choices.length > 0) {
						...
						dBug(choice, 2);
						dBug(choice.goesto);
						dBug(rooms[choice.goesto]);
						*/

						//eMaster('roomGeneric', this.data.location, {"sayFrom": sayFrom}, ` looks towards ${choice}`, client);

						eMaster('roomExit', this.data.location, {"sayFrom": sayFrom}, {"newRoom": choice.goesto, "exitTaken": choice}, client);
						eMaster('roomEnter', choice.goesto, {"sayFrom": sayFrom}, { "lastRoom": this.data.location, "exitTaken": choice }, client);
...
Mob.prototype.die = function(cause, battles, client) {
	...
	if (typeof cause === 'string') {
		causeString = cause;
	} else if (cause.constructor === Player) {
		...
		if (battles[player.id]) {
			// todo: battle needs to clean itself
			clearTimeout(battles[player.id].timer);
			delete battles[player.id];
		} else {
			dBug(`Weird! ${player.id} killed a ${this.data.type} outside of battle?`, 1);
		}
		...
		} else if (client) player.sendMsg(outP, client); // make sure client exists
	} else if (cause.constructor === Mob) {
		let mob = cause;
		causeString += mob.data.type;
		// update mob stats?
		if (battles[mob.inBattle]) {
			// well, the battle should know that a mob/player dies to delete itself
		} else {
			dBug(`Weird! A ${mob.data.type} killed a ${this.data.type} outside of battle?`, 1);
		}
...
const MobGenerator = function(mobTemplate, data, client) {
	this.data = Object.assign({}, data); // break first level of references
...
const buildPlayers = function(client) {
	...
	for (let player in players) {
		...
		let thePlayer = new Player(players[player], rooms, items, resources, client);
		...
		if (thePlayer.location === "airport") {
			thePlayer.location = "Loiterers Lounge"
		}
...
const buildItems = function(client) {
	...
	for (let playerId in players) {
		if (!players[playerId].inventory) {
			dBug(`${playerId} had no inventory, creating!`, 1);
		...
		if (weaponItemId) {
			...
			if (!itemTypes[iType]) {
				dBug(`${playerId} was carrying a weapon of non-existent type ${iType} - ignoring!`, 1);
			} else {
		...
		for (let itemId in players[playerId].inventory) {
			...
			if (!itemTypes[iType]) {
				dBug(`${playerId} was carrying an item of non-existent type ${iType} - ignoring!`, 1);
			} else {
	...
	for (let roomId in rooms) {
		for (let itemId in rooms[roomId].data.items) {
			...
			if (!itemTypes[iType]) {
				dBug(`Room ${roomId} contained an item of non-existent type ${iType} - ignoring!`, 1);
			} else {
...
const buildMobs = function(client) {
	for (let roomId in rooms) {
		for (let mobId in rooms[roomId].data.mobs) {
			let mobTemplate = rooms[roomId].data.mobs[mobId];
			// undead check -- "Developer's Curse"
			// just check for rats for now?
			if (mobTemplate === "airport sewer rat") {
				dBug(`Making an undead rat in ${roomId}`);
				mobTemplate = "undead airport sewer rat";
			}

			if (!mobTypes[mobTemplate]) {
				dBug(`Room ${roomId} contained a mob of non-existent type ${mobTemplate} - ignoring!`, 1);
			} else {
	...
	if (weaponId) {
		if (items[weaponId]) {
			...
			if (weapon) {
				...
				dBug(`battle participant had ${weaponId}, but that's not an Itemtype!`, 2);
			}
		} else {
			dBug(`battle participant had a non-existent weapon of ID ${weaponId}`, 2);
		}
	}
	return null;
};
...
const battles = {};
const BattleParticipant = function(participant, data, client) {
	...
	if (client) {
		dBug("Hey, client's defined.", 1);
	} else {
		dBug("Hey, client's NOT DEFINED!!!", 1);
	}
};
...
BattleParticipant.prototype.takeDamage = function(damageDealt, dmgType) {
	if (this.participant.constructor === Mob) {
		...
		parseScript([`grant stat hp -${damageDealt}`], {"who": this.participant.id}, this.participant.client);
	}
};
		...
		if (mobAbsorb) {
			if (Math.random() < mobAbsorb.chance) {
				if (mobAbsorb.percent) {
					...
					dBug(`${damageDealt} damage dealt after absorb`);
					damageDealt = Math.max(damageDealt, 0);
					dBug(`damageDealt is now ${damageDealt}`);
				} else {
					dBug(`${this.participant.data.type} has .absorb but no percent or hp property???`, 1);
				}
			}
		}
		return damageDealt;
...
BattleParticipant.prototype.doAction = function(battle, client) {
	...
	if (weapon) {
	...
	} else if (this.participant.constructor === Mob) {
		weapon = {
		...
		dBug("Non-player or mob in combat trying to find a weapon. This shouldn't happen.");
		return;
	}
    ...
    if (nextAction[0] === 'flee') {
        ...
        if (this.participant.constructor === Mob) {
            ...
            if (this.lastAggressor) {
                fleeSuccess = this.participant.flee(battle.participants[this.lastAggressor].participant, exitTaken, battles, this.participant.client);
            } else {
				dBug(this.target);
                fleeSuccess = this.participant.flee(battle.participants[this.target].participant, exitTaken, battles, this.participant.client);
            }
        }
        if (fleeSuccess) {
            if (this.lastAggressor) {
            ...
            // player.sendMsg(battleStr, client); // removed so that the player can witness the rest of the battle for this segment
            return;
		} else {
		...
		battleStr += `**${this.getName()}** attacks **${battle.participants[this.target].getName()}** with its ${weapon.weaponName}`;
		...
		dBug(`${this.getName()}'s hitChance is ${hitChance}`);

		if (Math.random() < hitChance) {
			let damageDealt = ut.rollDice(weapon.dmg);
			damageDealt = battle.participants[this.target].handleAbsorb(damageDealt);
			if (damageDealt <= 0) {
				battleStr += ` but the ${battle.participants[this.target].getName()} seems unharmed.\n`;
			} else {
			...
			dBug(`${this.getName()} hit ${battle.participants[this.target].getName()} for ${damageDealt}`);
		} else {
...
BattleParticipant.prototype.checkDeath = function(battle, client) {
	let death = false;
	if (this.participant.constructor === Mob) {
	...
	if (death) {
		// we need the last hitting party
		// TODO: handle mob cause of death lacking client variable (done)
		// DONE: Now we use client already attached to Player object.
		if (this.participant.constructor === Mob) {
			// DONE: this should give the player when the player attacks a mob
			this.participant.die(battle.participants[this.lastAggressor].participant, battles, client);
		} else if (this.participant.constructor === Player) {
			// This gives the mob name to a player
			this.participant.die(battle.participants[this.lastAggressor].getName(), battles, client);
		}
		//if (this.participant.client) this.participant.die(battle.participants[this.lastAggressor].getName(), battles, this.participant.client);
		//this.participant.die(battle.participants[this.lastAggressor].participant, battles, battle.participants[this.lastAggressor].participant.client);
		this.leftBattle = true;
		return true;
	}
	return false;
...
{
	segment: 7,
	battleOrder: ["167711491078750208", "The Klingon##7"]
	participants: [
		{
			participant: Player object 167711491078750208
			nextActionSegment: 10,
			nextAction: "flee",
			faction: "player"
		},
		{
			participant: Mob object
			nextActionSegment: 9,
			nextAction: "attack 167711491078750208",
			faction: "mob"
		}
	]
...
*/
const newBattleSegment = function(battle, client) {
	battle.segment++;
	...
	battle.battleOrder.forEach(participant => {
		if (battle.segment >= battle.participants[participant].nextActionSegment()) { // >= in case switch to a weapon with a smaller delay
			battleMessage += battle.participants[participant].doAction(battle, client);
		}
	});
	// for each participant, handle death and leaving battle (fleeing)
	battle.battleOrder.forEach(participant => {
		if (battle.participants[participant].checkDeath(battle, client)) {
	...
	if (battleMessage) {
		battle.battlingPlayers.forEach(player => {
			player.sendMsg(battleMessage, client);
		});
	}
    ...
    if (!battle.checkTerminated()) {
        battle.timer = setTimeout(function() {
            newBattleSegment(battle, client);
        }, cons.COMBAT.segDelay);
    } else {
        battle.end();
    }
};
const battleSegment = function(battle, client) {
	let participant;
	let target = battle.target;
	let player = players[battle.attacker];
	let battleStr = '';
	let damageDealt = {
		player: 0,
		mob: 0
	...
	// Handle player attacks (if needed)
	participant = player;



	if (participant.weapon) {
		if (items[participant.weapon]) {
			let weaponItemType = items[participant.weapon].data.type;
			weapon = itemTypes[weaponItemType]; // an ItemType object
			if (!weapon) {
				dBug(`battle participant had ${participant.weapon}, but that's not an Itemtype!`, 2);
			}
		} else {
			dBug(`battle participant had a non-existent weapon of ID ${participant.weapon}`, 2);
		}
	}
	...
	// for each, see if currentSegment is actionSegment
	// if so, handle their action


	if (battle.segment % delay === 0) {
		// it's the player's turn to act
		// so, figure out what they're doing

		let nextAction = battle.nextAction[battle.attacker].split(' ');
		...
		if (nextAction[0] === 'flee') {
			...
			let exitTaken = rooms[player.location].data.exits[exitTakenName];

			//let targetRoomName = rooms[player.location].exits[exitTaken].goesTo;
			//let targetRoom = rooms[targetRoomName];

			let fleeSuccess = player.flee(target, exitTaken, battles, client);

			if (fleeSuccess) {
				battleStr += `\n:runner: You have successfully fled from ${target.data.type}!\n`;
				player.sendMsg(battleStr, client);
				return;
			} else {
				battleStr += `:warning: _You tried to flee from the ${target.data.type}, but were unable!_`;
			}
		} else {
			// handle attacking (if it's not a flee, it's attack, for now)
			// TODO: (?) Refactor into something like player.attack()

			battleStr += `**${player.charName}** attacks **${target.data.type}** with its ${weaponName}`;

			let defense = target.data.defense;
			if (attack > defense) {
				hitChance = 0.5 + 0.5 * (1 - defense / attack);
			} else {
				hitChance = 0.5 * (attack / defense);
			}

			dBug(`${player.charName}'s hitChance is ${hitChance}`);

			if (Math.random() < hitChance) {
				let defense = target.data.defense;
				let damageDealt = ut.rollDice(dmg);

				// handle absorb
				let mobAbsorb = target.data.absorb;
				if (mobAbsorb) {
					if (Math.random() < mobAbsorb.chance) {
						if (mobAbsorb.percent) {
							damageDealt -= Math.floor(damageDealt * mobAbsorb.percent);
							...
							dBug(`${target.data.type} has .absorb but no percent or hp property???`, 1);
						}
					}
					if (damageDealt <= 0) {
						battleStr += ` but the ${target.data.type} seems unharmed.`;
					}
				}

				if (damageDealt) {
					battleStr += ` and hits for ${damageDealt} damage (${dmgType})!`;
				}

				target.takeDamage(player, damageDealt, dmgType);
				dBug(`${player.charName} hit ${target.data.type} for ${damageDealt} (${target.data.hp} / ${target.data.maxHp})`);
	...
	// Handle mob attacks (if needed)
	if (battle.segment % target.data.attack.delay === 0) {
		battleStr += `\n**${target.data.type}** attacks **${player.charName}** with its ${target.data.attack.name}`;
		dmg = target.data.attack.dmg;
		attack = target.data.attack.attack;
		defense = player.stats.defense || cons.COMBAT.defaultDefense;
		...
		dBug(`${target.data.type}'s hitChance is ${hitChance}`);

		let dmgType = target.data.attack.dmgType;
		if (Math.random() < hitChance) {
			damageDealt = ut.rollDice(dmg);
			battleStr += ` and hits for ${damageDealt} damage (${dmgType})!`;
			parseScript([`grant stat hp -${damageDealt}`], {"who": player.id}, client);
		} else {
			battleStr += ` but fails to hit!`;
		}
	}

	// Check for deaths, handle after battle string output
	targetDeath = (target.data.hp <= 0);
	playerDeath = (player.stats.hp.current <= 0);

	// Output the results of this segment of battle (if needed)
	if (battleStr !== '') {
		player.sendMsg(battleStr, client);
	}

	if (targetDeath) {
		target.die(player, battles, client);
		player.inBattle = false;
	} else {
		// Setup timer for next battle segment
		battle.timer = setTimeout(function() {
			battleSegment(battle, client);
		}, cons.COMBAT.segDelay);
	}

	if (playerDeath) {
		player.die(target.data.type, battles, client);
	}
};
//-----------------------------------------------------------------------------
...
module.exports = {
			...
			if (typeof rooms[pLoc].data.exits[where] !== 'undefined') {
				if (!rooms[pLoc].data.exits[where].goesto) {
					...
					if (!battles[player.inBattle]) {
						dBug(`Player.go(): players.${who}.inBattle is truthy, but there's no battle! Resetting to false!`, 2);
						player.inBattle = false;
						return;
					}
					...
					let exitTaken = rooms[pLoc].data.exits[where]; // find our target room
					chanStr += player.go(exitTaken, client); // actually move the character
				}
			} else {
	...
	flee: {
        do: function(message, args, client) {
            ...
            if (battles[who]
			  && battles[who].participants[who]
			  && battles[who].participants[who].target
			  && battles[who].participants[battles[who].participants[who].target]) {
                theMob = battles[who].participants[battles[who].participants[who].target].participant;
                if (theMob.constructor !== Mob) {
                    theMob = null;
                }
            }
			...
			// Check for unfleeables? (though 1 in fleeDifficulty does this)

			ut.chSend(message, "_You try to get away from combat..._");
			battles[who].participants[who].nextAction = `flee ${exitTaken}`;
		}
	},
	exits: {
		do: function(message, args, client) {
			...
			if (!exitText) {
				dBug(`SpongeMUD: Room \`${room.data.id}\` missing exits!`, 1);
			} else {
				outP += exitText;
			}
			ut.chSend(message, outP);
	...
	exit: {
		do: function(message, args, client) {
			...
			if (isNaN(choice) || choice < 1 || !exits[choice - 1]) {
				...
				outP += player.go(exitTaken, client);
			}
			ut.chSend(message, outP);
		}
	},
	...
	joinmud: {
		do: function(message, args, client) {
			...
			if (typeof player === 'undefined') {
				...
				player = new Player({charName: charName, id: who, posture: "standing", server: server.id}, rooms, items, resources, client);
				players[who] = player;
				ut.saveObj(players, cons.MUD.playerFile, {getData: true});
	...
	unwield: {
		do: function(message, args, client) {
			...
			if (!theItemId) {
				...
				if (!theItem) {
					dBug(`Couldn't unwield invalid item ${theItemId}!`);
				...
				if (choiceList[choiceNum].where === 'char') {
					...
					if (theMob.data.isEssential || !theMob.data.attack) {
						...
						if (battles[who]) {
							...
							dBug('HEY STARTING A NEW BATTLE, HERE IS CLIENT:');
							if (typeof client !== 'undefined') {
								dBug(' Do not fear, client is not undefined.');
							}
							...
							battles[battleId] = new Battle({
								...
								timer: setTimeout(function() {
									...
									newBattleSegment(battles[battleId], client);
								}, cons.COMBAT.segDelay)
							});
							/*
							battles[who] = new Battle({
								attacker: who,
								target: theMob,
								timer: setTimeout(function() {
									ut.chSend(message, `Combat with the ${battles[who].target.data.type} begins!`);
									battleSegment(battles[who], client);
								}, cons.COMBAT.segDelay)
							});
							*/
	...
	inv: {
		do: function(message, args) {
			...
			if (pl.weapon) {
				...
				if (!weapon) {
					dBug(`${pl.charName} had an invalid item ${pl.weapon}`, 2)
				} else {
					weaponName = weapon.data.type;
				}
			}
	...
	who: {
		do: function(message, args) {
			...
			if (player) {
				...
				ut.chSend(message, 'Sorry, I couldn\'t find ' + args);
			}
		}
	},
	nuke: {
		...
		do: function(message, args) {
			...
			if (!target) {
				if (!claims) {
					...
					if (!hadClaims) { outP += " _(no claims)_\n"; }
					outP += `(${player.stats.gather.current} / ${player.stats.gather.max} gathering points)`;
				}
			} else {
			...
			if (validGame) {
				...
				if (minigames[validGame].commands.hasOwnProperty(gameCmd)) {
					...
					if (readyStatus.isReady) {
						...
						if (result.script) {
							dBug(`Minigame "${game}" is running a MEHscript on exit...`);
							dBug(result.script);
							parseScript(result.script, {"who": who}, client);
						}
		...
		do: function(message, args) {
			...
			if (subCmd === 'read') {
				if (!msgNum || msgNum < 0 || msgNum > messages.length) {
					...
					outP += `SUBJECT: ${msg.subject}\n-----\n`
					outP += msg.contents;
					outP += "```";
				}
			} else if (subCmd === "send") {
				outP += "If you want to send mail, you'll need to go to a Postpigeon's Roost.\n";
				outP += "Once there, use the command post <character> <subject> to create an envelope.\n";
				outP += "Then, type `use envelope <message>` to to write your message and send it off.";
		...
		do: function(message, args) {
			...
			if (!receipient) {
				...
				if (!subject) {
					...
					let env = player.mailEnvelope;
					if (!env) {
					} else {
						outP += `You are already working on a letter for ${player.mailEnvelope}. Finish that first.\n`;
