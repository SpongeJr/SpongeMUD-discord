...
const defaultRoomDescribe = function(viewAs) {
	...
	for (let mobId in roomMobs) {
		...
		if (opponent) {
			outStr += `, fighting **${players[opponent].charName}**!`;
		} else {
			outStr += `.`;
		}
	}
		...
		do: function(message, args, client) {
			...
			if (typeof rooms[pLoc].data.exits[where] !== 'undefined') {
				if (!rooms[pLoc].data.exits[where].goesto) {
					...
					if (!battles[who]) {
						dBug(`Player.go(): players.${who}.inBattle is true, but there's no battle! Resetting to false!`, 2);
						player.inBattle = false;
						return;
					} 
					
					let target = battles[who].target.data.type;
					ut.chSend(message, `You can't leave, the ${target} you are fighting won't let you escape!`);
					return;
				} else {
						...
						if (battles[who]) {
							...
							battles[battleId] = new Battle({
								...
								timer: setTimeout(function() {
									...
									newBattleSegment(battles[who], client);
								}, cons.COMBAT.segDelay)
							});
							/*
