...
Mob.prototype.takeDamage = function(cause, dmgAmt, dmgType, client) {
...
const battleSegment = function(battle, client) {
	...
	if (player.weapon) {
		...
		if (battle.segment % delay === 0) {
			battleStr += `**${player.charName}** attacks **${target.data.type}** with its ${type}`;
			if (Math.random() < hitChance) {
				let damageDealt = ut.rollDice(dmg);
				battleStr += ` and hits for ${damageDealt} damage (${type})!`;
				target.takeDamage(player, damageDealt, type, client);
