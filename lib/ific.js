...
const buildPicklist = function(itemList, matchStr) {
	...
	// 		I guess just as long as whatever gets sent up, it looks like:
	// matchStr = string to match against
	
	//console.log(`buildPicklist(): itemList is: ${JSON.stringify(itemList)}`);
	...
	wheres.forEach(function(where) {	
		...
		for (var itemId in itemList[where]) {
			...
			// check against shortName (later against shortNames[])
			
			var matchFound = false;
			var matchNum = 0;
			var match = -1;
			
			while (!matchFound && matchNum < shortNames.length && shortNames) {		
	...
	var pickNum = 0;
	choiceList = [];
	for (var where in choices) {
		var numStr = '';
		for (var num = 0; num < choices[where].length; num++) {
			//numStr = (pickNum === 0) ? '' : (pickNum + 1) + '.';
			choiceList.push({
	...
	switch (action) {
		...
		case 'tele':
			if (!player.isAtLeast('wizard')) {
				return "You're not wizardly enough to teleport! (tele is now limited to Wizard+)";
			}
			break;
		case 'profile':
...
var defaultPlayerDescribeAs = function(viewAs) {
	...
	if (zone) {
		if (zoneList[zone]) {
			...
			console.log(`WARNING: room ${this.location} has non-existent zone ${zone}!`);
		}
	}
...
var defaultItemEventHandler = function(eventName, callback) {
	...
	if (eventName === 'worldTick') {
		...
		console.log(`WARNING: Unknown event ${eventName} triggered on ${id}`);
	}
};
var defaultItemEventKiller = function(eventName) {
	...
	if (eventName === 'worldTick') {
		...
		console.log(`WARNING: Tried to kill unknown event ${eventName} on ${id}`);
	}
};
var nextId = {};
//-----------------------------------------------------------------------------
...
var SceneryItem = function(itemType, data) {
	...
	this.id = itemType + '##' + nextId[itemType];
	console.log(`Prop ${this.data.shortName} created with id: ${this.id}. Placing in: ${this.data.location}`);
	// figure out where it belongs (room or player) and update that object, too
	var loc = this.data.location;
	if (isNaN(loc.charAt(0))) {
		// first letter not a number, so it's in a room
...
Player.prototype.unlockTitle = function(titleNum) {
	...
	if (this.stats.unlockedTitles.indexOf(titleNum) !== -1) {
		console.log('INFO: ${this.id}.unlockTitle(${titleNum): Already had it.');
		return;
	} else {
		this.stats.unlockedTitles.push(titleNum);
		console.log('INFO: ${this.id}.stats.unlockedTitles is now: ${${this.stats.unlockedTitles}');
	}
}
...
Player.prototype.registerForWorldTicks = function(client) {
	...
	this.on('worldTick', function() {
		...
		if (!player.idle) {
			...
			console.log(`INFO: created .idle for players.${player.id}.`);
		}
		
		player.idle.ticks++;
		//console.log(`${player.charName}.idle.ticks = ${player.idle.ticks} / ${player.idle.threshhold}`);
		...
		if (player.idle.ticks > player.idle.threshhold) {
			...
			if (player.idle.autolog) {
				...
				console.log(`[ IDLE TIMEOUT ] Just forced ${player.charName} to .sleep() (>${player.idle.threshhold} ticks)`);
			}
		}
	});
};
...
var buildItems = function() {
	...
	for (let player in players) {
		...
		for (let itemId in players[player].inventory) {
			...
			if (!itemTypes[iType]) {
				console.log(`WARNING! ${player} was carrying an item of non-existent type ${iType} - ignoring!`);
			} else {
	...
	for (let room in rooms) {
		for (let itemId in rooms[room].data.items) {
			...
			if (!itemTypes[iType]) {
				console.log(`WARNING! Room ${room} contained an item of non-existent type ${iType} - ignoring!`);
			} else {
...
var worldTick = function(client) {
	...
	if (now.remain === 0) {
		...
		console.log(`    o   The hour hand advances to ${now.hour.toString().padStart(2, ' ')}`);
...
module.exports = {
	...
	idleReset: function(message) {
		...
		if (!player) {
			console.log(`INFO: idle timeout checker found no players.${who}.`);
		} else {
			if (!player.idle) {
				player.idle = {
					ticks: 0,
				...
				console.log(`INFO: created .idle for players.${who}.`);
			}
	...
	autolog: {
		do: function(message, args) {
			...
			if (player.idle.autolog) {
				ut.chSend(message, `${player.charName} will be logged out ` +
				`after ${player.idle.threshhold} ticks of no MUD activity.`);
			} else {
				ut.chSend(message, `${player.charName} will no longer be automatically logged out when idle.`);
			}
		}
	},
	age: {
	...
	exitmud: {
		do: function(message, parms, client) {
			...
			if (typeof players[who] === 'undefined') {
				...
				player.server = null; // TODO: remember why this is here
				player.sleep(); // Zzz
				// we can fire off a "X snores" because they should be unregistered now
				let phrase = ut.listPick(["drifts off to sleep", "closes their eyes and immediately starts snoring",
					"falls asleep", "nods off to sleep", "falls into a deep slumber"]);

				eMaster('roomGeneric', player.location, {"sayFrom": player.charName}, 'drifts off', client);	
	...
	publish: {
		do: function(message, args) {
			...
			if (!templates || !templates[0]) {
				...
				player.carriedTemplates = [];
				console.log(` (( PUBLISH ))   ${player.charName} just published ${totalTemps} pending template(s)`);
				}
				ut.chSend(message, outP);
		}
	},
		...
		do: function(message, args) {
			...
			if (!players.hasOwnProperty(target)) {
				...
				if (!players[target].stats.accessLevel) {
					console.log(`INFO: players.${target} did not have a .stats.accessLevel until now.`);
				}
