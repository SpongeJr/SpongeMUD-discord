...
var defaultRoomDescribe = function(viewAs) {
	...
	if (this.data.hasOwnProperty('items')) {	
		...
		for (var item in items) {
			if (items[item].data.family === 'mobile') {
				mobs[item] = items[item];
			}
		}
		
		for (var mob in mobs) {
			if (mobs[mob].data.location === this) {
				outStr += `**${mobs[mob]}** is here. `;
			}
		}
		outStr += mobStr;
	}
...
var defaultRoomShortDesc = function(viewAs) {
	...
	var outStr = `-= SP: ${viewAs.stats.stamina}/${viewAs.stats.maxStamina}`;
	outStr += `  XP: ${viewAs.stats.xp}   dayTicks: ${viewAs.stats.dayTicks}  =-\n`;
	outStr += '**' + this.data.title + '**';
