...
BattleParticipant.prototype.nextActionSegment = function() {
    ...
    if (this.participant.constructor === Mob) {
        return this.previousActionSegment + this.participant.data.attack.dmg;
...
BattleParticipant.prototype.doAction = function(battle) {
    ...
    if (nextAction[0] === 'flee') {
		...
		if (Math.random() < hitChance) {
			...
			if (damageDealt <= 0) {
				battleStr += ` but the ${this.participants[this.target].getName()} seems unharmed.\n`;
			} else {
...
module.exports = {
	...
	flee: {
        do: function(message, args, client) {
            ...
            let player = players[who];
            let theMob = null;
            if (battles[who] && battles[who].participants && battles[who].participants[who] && battles[who].participants[who].target && battles[who].participants[battles[who].participants[who].target]) {
