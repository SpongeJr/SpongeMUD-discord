...
const defaultTimerCheck = function(player, world, cmd) {
	...
	if (!nextUse || (nowTick >= nextUse)) {
		...
		let returnOn = ut.timeDiffStr(next);
			...
			do: function(player, world, args) {
				...
				if (success) {
					...
					if (result.isIdeal) {
						...
						// grant Edibles reward if configured to do so
						if (Object.keys(v.gameCfg.edibleAwards.correct).length !== 0) {				
							message += '\nThe Troll Chef also sends some Edibles materials to your resource stores!\n( ';
							for (let rarity in v.gameCfg.edibleAwards.correct) {
								materialAmount = v.gameCfg.edibleAwards.correct[rarity];
								edibleScript.push(`grant material edibles ${rarity} ${materialAmount}`);
								message += `${materialAmount} ${rarityStrings[rarity]}  `;
							}
							message += ')';
							
						}
					} else {
						// grant them a small token award of Edibles if configured to do so
						if (Object.keys(v.gameCfg.edibleAwards.incorrect).length !== 0) {
							message += '\nFor making an attempt, the Troll Chef sends you on your way ';
							message += 'with a small contribution to your Edibles resource stores.\n( ';
							for (let rarity in v.gameCfg.edibleAwards.incorrect) {
								materialAmount = v.gameCfg.edibleAwards.correct[rarity];
								edibleScript.push(`grant material edibles ${rarity} ${materialAmount}`);
								message += `${materialAmount} ${rarityStrings[rarity]}  `;
							}
